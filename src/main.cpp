...
CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 5); // PoW starting difficulty on Testnet
CBigNum bnProofOfWorkFirstBlock(~uint256(0) >> 30);

unsigned int nTargetSpacing = 1 * 60; // 60 seconds
unsigned int nRetarget = 20;
unsigned int nStakeMinAge = 24 * 60 * 60; // 12 hours
unsigned int nStakeMaxAge = 30 * 24 * 60 * 60;           // 30days
...
const string strMessageMagic = "microCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // microCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::IsStandard() const
{
    ...
    if (nDataOut > nTxnOut) return false;
                        ...
                        bool* pfMissingInputs)
{
    ...
    // microCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// microCoin block size.
    int64_t nSubsidy = 0 * COIN;
        ...
        if(pindexBest->nHeight >= 1 && pindexBest->nHeight <= 100)    
        {
                nSubsidy = 308500000 * COIN;		// Amount of the old microCoin network+ one month mining time ~ 30850000000. All coins changed with ratio 1:1 in alcurEX exchange.
        }
		else if(pindexBest->nHeight > 100 && pindexBest->nHeight <= 5000)    
        {
		 nSubsidy = 1000 * COIN; // 5000 blocks PoW blocks to get network working stable with PoS only.
		}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    int64_t nRewardCoinYear;
		...
		{
		nRewardCoinYear = REST_MINT_PROOF_OF_STAKE;
		}		
...
// microCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes microCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // microCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // microCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // microCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // microCoin: track money supply and mint amount info
...
// microCoin: total coin age spent in transaction, in the unit of coin-days.
...
// microCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // microCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // microCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // microCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // microCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // microCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // microCoin: check proof-of-stake
    ...
    // microCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // microCoin: getblocks may not obtain the ancestor block rejected
    ...
    // microCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "microCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "microCoin relaunch.";
        CTransaction txNew;
        txNew.nTime = 1410880428;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1410880428;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1088460;
        if(fTestNet)
        {
            block.nNonce   = 18;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x43a7b895fc093e79860e47a821a49fc49f035393b4ee576c785862e6461e069c"));
        ...
        // microCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // microCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // microCoin: tell downloading node about the latest block if it's
